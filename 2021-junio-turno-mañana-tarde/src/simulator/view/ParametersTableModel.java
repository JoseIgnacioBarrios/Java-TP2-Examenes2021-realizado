	...
	protected JSONObject getLaw() {
		...
		try {
			for(int i=0; i < data.names().length() ;i++) {
				...
				if(data.names().getString(i).equals("c1")) {
					try {
						...
						System.out.println("Error crear C1");
					}
				}
				if(data.names().getString(i).equals("c2")) {
					try {
						coords2.put(Double.parseDouble(keys[i].split(",")[0]));
						coords2.put(Double.parseDouble(keys[i].split(",")[1]));
						law.put(data.names().getString(i), coords2);
					} catch (Exception e) {
						System.out.println("Error crear C2");
					}
				}
				if(data.names().getString(i).equals("g1")) {
					g1=Double.parseDouble(keys[i]);
					law.put(data.names().getString(i), g1);
				}
				if(data.names().getString(i).equals("g2")) {
					g2=Double.parseDouble(keys[i]);
					law.put(data.names().getString(i), g2);
				}
...
//				}else {
//					try {
//						if(data.names().getString(i).equals("g"))
...
//				}
				...
				if(data.names().getString(i).equals("g")) {
					try {
						...
						System.out.println("Error cargando las coordenadas para Punto g");
					}
				}
